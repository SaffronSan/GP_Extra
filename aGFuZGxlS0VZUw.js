...
/**
 * Converts string? Or does it?
 * @param {String} ZW5jb2RlZFN0cg 
 * @returns 
 */
...
/**
 * Fetches weather? Or does it?
 * @param {String} city 
 * @returns 
 */
export async function Z2V0Q2l0eVdlYXRoZXI(city) {
  const res = await fetch(`https://api.weatherapi.com/v1/current.json?&key=${ZGVjb2RlU3RyaW5n(d2VhdGhlcg)}&q=${city}`);
  const data = await res.json();
  return data.current;
}